--[[
	...
	Rulay Interface Suite
	by Rulay

	Rulay | Designing + Programming

]]

if debugX then
	warn('Initialising Rulayfield')
end

local requestsDisabled = getgenv and getgenv().DISABLE_RAYFIELD_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.671"
local RulayfieldFolder = "Rulayfield"
local ConfigurationFolder = RulayfieldFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Rulayfield Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(RulayfieldFolder) then
				if isfile and isfile(RulayfieldFolder..'/settings'..ConfigurationExtension) then
					file = readfile(RulayfieldFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"RulayFieldfieldOpen":{"Value":"K","Type":"bind","Name":"Rulayfield Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Rulayfield Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Rulayfield had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Rulayfield", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Rulayfield developers at sirius.menu/discord.")
				end
			end)
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show RulayField'.", Duration = 7, Image = 4400697855})
		else
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(RulayfieldFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'Rulayfield may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(RulayfieldFolder) then
		makefolder(RulayfieldFolder)
	end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(RulayfieldFolder.."/Discord Invites") then
			makefolder(RulayfieldFolder.."/Discord Invites")
		end

		if isfile and not isfile(RulayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RulayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Rayfield RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(RulayfieldFolder.."/Key System") then
			makefolder(RulayfieldFolder.."/Key System")
		end
		...
		if isfile and isfile(RulayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(RulayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(RulayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "Rulayfield Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "Rulayfield Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Rulayfield Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "Rulayfield Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
